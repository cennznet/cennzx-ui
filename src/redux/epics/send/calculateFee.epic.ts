...
import {setExchangeError, updateTransactionFee} from '../../actions/ui/send.action';
...
export const calculateTxFeeEpic = (
    ...
    combineLatest([api$, action$.pipe(ofType(types.ui.Send.TRANSACTION_FEE_PARAMS_UPDATE))]).pipe(
        withLatestFrom(store$),
        switchMap(
            ([[api, action], store]): Observable<Action<any>> => {
                const {txFee, extrinsicParams} = store.ui.send;
                const {extrinsic, signingAccount, feeAssetId} = store.ui.send.form;
